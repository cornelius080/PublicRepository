...
namespace NPREth_GetSetConfiguration
{
    public class NPREth_Connect
    {
        ...
        public NPR_NetParameters netParameters = null;
        ...
        public DeviceParameters deviceParameters; //net and manufacturer parameters
        ...
        public NPREth_Connect()
        {
            ...
            Init();
            netParameters = new NPR_NetParameters(serverSocket);
        }
        ...
        {
            //TASK FOR READING THE NET PARAMETERS OF THE DEVICE
            Task<DeviceParameters> DeviceParameterTask = Task<DeviceParameters>.Factory.StartNew(() =>
            {
                DeviceParameters dp;
                dp = netParameters.GetNetParameters();
                return dp;
            });
            deviceParameters = DeviceParameterTask.Result;            
            ...
            try
            {
                while (serverSocket.Connected && isConnected)
                {
                    ...
                    ////////////////////////////
                    //REC accetta solo multipli di quattro; in caso contrario, restituisce un numero arrotondato
                    SendToNPR("REC 4");
                    Thread.Sleep(200);
                    ReadFromNPR();
        ...
        //private void NPR_SendCommand(int nBytes)
        //{
        //    string stringCommand = "SEND " + nBytes.ToString() + "\r\n";
        //    //byte[] byteCommand = Encoding.ASCII.GetBytes(stringCommand);
        //    int byteSend = serverSocket.Send(Encoding.ASCII.GetBytes(stringCommand));
        //    serverSocket.Receive(answerReceived, nBytes, SocketFlags.None);

        //    if (String.Equals(Encoding.ASCII.GetString(answerReceived), succString))
        //    {
        //        serverSocket.Send(Encoding.ASCII.GetBytes(nBytes.ToString() + "\r\n"));
        //    }
        //    else
        //    {
        //        MessageBox.Show("NO DATA WRITTEN. SEND COMMAND RETURNS: ERR\r\n");
        //    }
        //}
